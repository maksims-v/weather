...
let removeCity;
let removeTaskArray;
if (localStorage.length === 0) localStorage.setItem('favoriteList', JSON.stringify([
    ...setOfFavoritycity
]));
else {
    ...
    favoriteCityListArray.forEach((city1)=>{
        setOfFavoritycity.add(city1);
    });
    setOfFavoritycity.forEach((city2)=>{
    ...
    removeTaskArray.forEach((city3, i)=>{
...
_viewJs.FORM.addEventListener('submit', (e)=>{
    e.preventDefault();
...
addFavoriteCity.addEventListener('click', (e1)=>{
    e1.preventDefault();
    ...
    setOfFavoritycity.forEach((city4)=>{
        favoriteCityList.innerHTML += `<li class="city-list__item">
      <button class="city-list__item-btn">${city4}</button>
      <div class="remove_button">DELETE</div>
    </li>`;
    });
    removeCity = document.querySelectorAll('.remove_button');
    removeTaskArray = Array.prototype.slice.call(removeCity);
    removeTaskArray.forEach((city5, i)=>{
        city5.addEventListener('click', (e)=>{
            e.target.parentNode.remove();
            setOfFavoritycity.delete(e.target.parentNode.firstElementChild.innerText);
            removeTaskArray.splice(0, 1);
            localStorage.setItem('favoriteList', JSON.stringify([
...
async function getData(cityInput) {
    ...
    let localTime = await fetch(`${serverUrl}?q=riga&appid=${apiKey}&units=metric`);
    let localTimeData = await localTime.json();
    let localSunrisehours = new Date(localTimeData.sys.sunrise * 1000).getHours();
    let localSunriseminutes = new Date(localTimeData.sys.sunrise * 1000).getMinutes();
    let localSunsethours = new Date(localTimeData.sys.sunset * 1000).getHours();
    let localSunsetminutes = new Date(localTimeData.sys.sunset * 1000).getMinutes();
    console.log(localSunrisehours, localSunriseminutes);
    console.log(localSunsethours, localSunsetminutes);
    try {
        ...
        detailSunrise.innerText = `${sunrisehours}:${sunriseminutes}`;
        detailSunset.innerText = `${sunsethours}:${sunsetminutes}`;
    } catch (err) {
        console.log(err);
    }
    fetch(urlForecast).then((response)=>response.json()
        ...
        dataForecast.list.forEach((item, i)=>{
            ...
            if (forecast12clock === 12) forecastArr.push(item);
        });
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["kQMTH","adjPd"], "adjPd", "parcelRequire94c2")
